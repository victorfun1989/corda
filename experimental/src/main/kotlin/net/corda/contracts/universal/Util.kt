...
private fun signingParties(perceivable: Perceivable<Boolean>) : ImmutableSet<PartyWithoutCertificate> =
    when (perceivable) {
        ...
        is TimePerceivable -> ImmutableSet.of<PartyWithoutCertificate>()
        is TerminalEvent -> ImmutableSet.of( perceivable.reference )
        is PerceivableComparison<*> -> ImmutableSet.of<PartyWithoutCertificate>() // todo
...
private fun involvedPartiesVisitor(action: Action): Set<PartyWithoutCertificate> =
    Sets.union(involvedPartiesVisitor(action.arrangement), signingParties(action.condition)).immutableCopy()

private fun involvedPartiesVisitor(arrangement: Arrangement): ImmutableSet<PartyWithoutCertificate> =
        when (arrangement) {
            is Zero -> ImmutableSet.of<PartyWithoutCertificate>()
            is Obligation -> ImmutableSet.of(arrangement.from, arrangement.to)
            is RollOut -> involvedPartiesVisitor(arrangement.template)
            is Continuation -> ImmutableSet.of<PartyWithoutCertificate>()
            is And ->
                arrangement.arrangements.fold(ImmutableSet.builder<PartyWithoutCertificate>(), { builder, k -> builder.addAll(involvedPartiesVisitor(k)) }).build()
            is Actions ->
                arrangement.actions.fold(ImmutableSet.builder<PartyWithoutCertificate>(), { builder, k -> builder.addAll(involvedPartiesVisitor(k)) }).build()
            else -> throw IllegalArgumentException(arrangement.toString())
        }

/** returns list of involved parties for a given contract */
fun involvedParties(arrangement: Arrangement): Set<PartyWithoutCertificate> = involvedPartiesVisitor(arrangement)

fun replaceParty(perceivable: Perceivable<Boolean>, from: PartyWithoutCertificate, to: PartyWithoutCertificate): Perceivable<Boolean> =
        when (perceivable) {
            is ActorPerceivable ->
...
fun replaceParty(action: Action, from: PartyWithoutCertificate, to: PartyWithoutCertificate): Action =
...
fun replaceParty(arrangement: Arrangement, from: PartyWithoutCertificate, to: PartyWithoutCertificate): Arrangement = when (arrangement) {
    is Zero -> arrangement
...
fun debugCompare(parLeft: PartyWithoutCertificate, parRight: PartyWithoutCertificate) {
...
fun debugCompare(parLeft: Set<PartyWithoutCertificate>, parRight: Set<PartyWithoutCertificate>) {
